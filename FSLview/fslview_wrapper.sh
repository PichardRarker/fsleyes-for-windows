...
function script_help {
  echo '##################################################################################################'
  echo 'File:                   fslview_wrapper.sh'
  echo ""
  echo 'Description:            Simple wrapper for FSLview, designed to be used in WSL. Script initialises'
  echo '                        FSL, sets up some logfiles, then launches FSLview'
  echo ""
...
export LOGFILE=$ROOTDIR/fslview_wrapper.log 
...
#Launch FSLeyes!
$FSLDIR/bin/fslview_deprecated $NII_PATH_WSL >> $LOGFILE 2>&1
